...
def ransac_line(p3ds):
    ...
    for p3d in p3ds:
        ...
        # A: 3D slope, B: Axis interception, inliers: inliers' index
        A, B, inliers = line.fit(p3d, thresh=0.2, maxIteration=1000)
        coes.append(A)
...
# 杆状物点云聚类
# open3d DBSCAN
def pole_cluster(p3d):

    p3d = np.array(p3d)
    X= p3d
    
    dbscan=DBSCAN(eps=0.2,min_samples=30) #eps是半径
    dbscan.fit(X)

    labels = dbscan.labels_  #所有点的label
    n_clus = len(set(labels)) - (1 if -1 in labels else 0)   #一共有多少类

    # clus_labels = np.unique(labels)   #每一类的label

    p3d_clus = []
    for i in range(n_clus):
        p3d_temp = X[labels == i]
        p3d_clus.append(p3d_temp)
...
if __name__ == '__main__':
    ...
    # 获得2d图片上, label为ground 7的点和pole 17的坐标
    h = img_label.shape[0]
    w = img_label.shape[1]
    p_2d_road = []
    p_2d_pole = []
    for i in range(h):
        for j in range(w):
            if img_label[i,j]==7:
                p_2d_road.append([i,j])
            if img_label[i,j]==17:
                p_2d_pole.append([i,j]) 

    # import pdb; pdb.set_trace()
    # 利用深度图和内参，把2d地面点和pole恢复成3d相机坐标系上的点
    p_3d_road = projection(p_2d_road, extrinsic, intrinsic, img_dis)
    p_3d_road = [p for p in p_3d_road if p[2]<30]

    p_3d_pole = projection(p_2d_pole, extrinsic, intrinsic, img_dis)
    p_3d_pole = [p for p in p_3d_pole if p[2]<30]

    # 可视化
    # point_visualization(p_3d_road, [])
    point_visualization(p_3d_pole, [])

    # ransac算法估计出地面点的平面参数
    # coe = ransac_plane_sklearn(p_3d_road)
    coe_open3d_road = ransac_plane_open3d(p_3d_road)
    # print(y_loss)
    # print(coe)
    print(coe_open3d_road)


    # 通过聚类和直线拟合，得到pole参数
    poles_3d = pole_cluster(p_3d_pole)
    coe_poles = ransac_line(poles_3d)
