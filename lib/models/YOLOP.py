...
# 增加杆状物分割头
YOLOP_pole = [
[24, 33, 42, 51],   #Det_out_idx, Da_Segout_idx, LL_Segout_idx, PO_Segout_idx
[ -1, Focus, [3, 32, 3]],   #0
[ -1, Conv, [32, 64, 3, 2]],    #1
[ -1, BottleneckCSP, [64, 64, 1]],  #2
[ -1, Conv, [64, 128, 3, 2]],   #3
...
[ -1, Conv, [8, 2, 3, 1]] #51 Pole Object segmentation head
]
...
class MCnet_pole(nn.Module):
    def __init__(self, block_cfg, **kwargs):
        super(MCnet_pole, self).__init__()
        layers, save= [], []
        self.nc = 1
        self.detector_index = -1
        self.det_out_idx = block_cfg[0][0]
...
def get_net_pole(cfg, **kwargs): 
    m_block_cfg = YOLOP_pole
    model = MCnet_pole(m_block_cfg, **kwargs)
    return model
...
if __name__ == "__main__":
    ...
    detects, dring_area_seg, lane_line_seg, pole_object_seg = model_out
