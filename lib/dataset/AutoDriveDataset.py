...
from ..utils import letterbox, letterbox_pole, augment_hsv, random_perspective, xyxy2xywh, cutout, random_perspective_pole
...
class AutoDriveDataset_pole(Dataset):
    """
    A general Dataset for some common function
    """
    def __init__(self, cfg, is_train, inputsize=640, transform=None):
        ...
        mask_root = Path(cfg.DATASET.MASKROOT)
        lane_root = Path(cfg.DATASET.LANEROOT)
        pole_root = Path(cfg.DATASET.POLEROOT)
        if is_train:
            indicator = cfg.DATASET.TRAIN_SET
        else:
            indicator = cfg.DATASET.TEST_SET
        ...
        self.mask_root = mask_root / indicator
        self.lane_root = lane_root / indicator
        self.pole_root = pole_root / indicator
        # self.label_list = self.label_root.iterdir()
        self.mask_list = self.mask_root.iterdir()
    ...
    def __getitem__(self, idx):
        ...
        if self.cfg.num_seg_class == 3:
            ...
            seg_label = cv2.imread(data["mask"], 0)
        lane_label = cv2.imread(data["lane"], 0)
        pole_label = cv2.imread(data["pole"], 0)
        #print(lane_label.shape)
        # print(seg_label.shape)
        # print(lane_label.shape)
        # print(seg_label.shape)
        ...
        if r != 1:  # always resize down, only resize up if training with augmentation
            ...
            seg_label = cv2.resize(seg_label, (int(w0 * r), int(h0 * r)), interpolation=interp)
            lane_label = cv2.resize(lane_label, (int(w0 * r), int(h0 * r)), interpolation=interp)
            pole_label = cv2.resize(pole_label, (int(w0 * r), int(h0 * r)), interpolation=interp)
        h, w = img.shape[:2]
        
        (img, seg_label, lane_label, pole_label), ratio, pad = letterbox_pole((img, seg_label, lane_label, pole_label), resized_shape, auto=True, scaleup=self.is_train)
        shapes = (h0, w0), ((h / h0, w / w0), pad)  # for COCO mAP rescaling
        # ratio = (w / w0, h / h0)
        # print(resized_shape)
        ...
        if self.is_train:
            combination = (img, seg_label, lane_label, pole_label)
            (img, seg_label, lane_label, pole_label), labels = random_perspective_pole(
                combination=combination,
                targets=labels,
                degrees=self.cfg.DATASET.ROT_FACTOR,
                translate=self.cfg.DATASET.TRANSLATE,
            ...
            if lr_flip and random.random() < 0.5:
                ...
                seg_label = np.fliplr(seg_label)
                lane_label = np.fliplr(lane_label)
                pole_label = np.fliplr(pole_label)
                if len(labels):
                    labels[:, 1] = 1 - labels[:, 1]
            ...
            if ud_flip and random.random() < 0.5:
                ...
                seg_label = np.filpud(seg_label)
                lane_label = np.filpud(lane_label)
                pole_label = np.filpud(pole_label)
                if len(labels):
                    labels[:, 2] = 1 - labels[:, 2]
        ...
        _,lane1 = cv2.threshold(lane_label,1,255,cv2.THRESH_BINARY)
        _,lane2 = cv2.threshold(lane_label,1,255,cv2.THRESH_BINARY_INV)
        _,pole1 = cv2.threshold(pole_label,1,255,cv2.THRESH_BINARY)
        _,pole2 = cv2.threshold(pole_label,1,255,cv2.THRESH_BINARY_INV)
#        _,seg2 = cv2.threshold(seg_label[:,:,2],1,255,cv2.THRESH_BINARY)
        # # seg1[cutout_mask] = 0
        # # seg2[cutout_mask] = 0
        ...
        lane1 = self.Tensor(lane1)
        lane2 = self.Tensor(lane2)
        pole1 = self.Tensor(pole1)
        pole2 = self.Tensor(pole2)
        ...
        lane_label = torch.stack((lane2[0], lane1[0]),0)
        pole_label = torch.stack((pole2[0], pole1[0]),0)
        # _, gt_mask = torch.max(seg_label, 0)
        # _ = show_seg_result(img, gt_mask, idx, 0, save_dir='debug', is_gt=True)
        

        target = [labels_out, seg_label, lane_label, pole_label]
        img = self.transform(img)
    ...
    def collate_fn(batch):
        img, label, paths, shapes= zip(*batch)
        label_det, label_seg, label_lane, label_pole = [], [], [], []
        for i, l in enumerate(label):
            l_det, l_seg, l_lane, l_pole = l
            l_det[:, 0] = i  # add target image index for build_targets()
            label_det.append(l_det)
            label_seg.append(l_seg)
            label_lane.append(l_lane)
            label_pole.append(l_pole)
        return torch.stack(img, 0), [torch.cat(label_det, 0), torch.stack(label_seg, 0), torch.stack(label_lane, 0), torch.stack(label_pole, 0)], paths, shapes
